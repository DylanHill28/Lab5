...
public class Calculator
{
   ...
   */
   protected static int calculateTwoTokens(String[] tokens) throws NumberFormatException, CalculatorException
   {
       ...
       //based on command, calculate teh value
       switch(tokens[0].trim().toLowerCase()) {
           case "halve":    retValue = a /2;
                           break;
                          
           case "negate":    retValue = -retValue;
                           break;
                          
           default:       throw new CalculatorException("Invalid command - " + tokens[0]);
       }
   ...
   */
           ...
           throws ArithmeticException, NumberFormatException, CalculatorException
   {
       ...
       switch(command.trim().toLowerCase()) {
           ...
           default:
                       throw new CalculatorException("Invalid command - " + command);
       }
   ...
   */
   ...
   {
       //Based on the length, invoke the right methods
       ...
       switch(tokens.length)
       {
           ...
           case 1:      
                       if(tokens[0].trim().equalsIgnoreCase("quit")) {
   ...
   */
   public static String parseAndExecute(String input)
   {
       ...
       //Split the input string into tokens and execute it.
       ...
       try {
           ...
           int retVal = execute(tokens);
           if(retVal == Integer.MIN_VALUE) {
               return "quit";
           } else {
               return "The result is: " + retVal;
           }
           ...
           return "Calculator Exception, message is: " + ex.getMessage();
       }
   }
}